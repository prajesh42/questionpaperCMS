                    ...
                    <ol class="accordion" id="searchSelect" class="list-group" style='text-align: center;'>
                        <li class="list-group-item list-group-item-action"><a class="btn"  data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">Fields</a> 
                          <div id="collapseOne" class="collapse" aria-labelledby="headingOne" data-parent="#searchSelect">
                             <form method="POST">
                                  <input hidden name="search" value="<?php echo $fieldid; ?>">
                                  <button class="btn btn-link" name="find"><?php echo $fieldid; ?></button>
                              </form>
                          </div>
                        </li>
                ...
                <table class="table table-striped table-hover">
                    ...
                    <tbody class="table-success">
                        <?php if(isset($_POST['find'])){
                                 $search=$_POST['search'];
                                 $query="select * from questions where (field LIKE '%$search%' OR year LIKE '%$search%' OR branchname LIKE '%$search%' OR semester LIKE '%$search%' OR filename LIKE '%$search%') AND (field LIKE '$fieldid')";
                                if($result= mysqli_query($link, $query))
                                {
                                $count=0;
                    while($row= mysqli_fetch_array($result))
                   ...
                   ?>
                    ...
                    <?php 
                    $query="select * from questions where field LIKE '$fieldid'";
